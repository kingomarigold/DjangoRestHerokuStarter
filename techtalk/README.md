...
- [ ] Add an email address to the `ADMINS` settings variable in `techtalk/backend/techtalk/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `techtalk/backend/techtalk/settings/production.py`
...
- Inside the `backend` folder, do the following:
  - Create a copy of `techtalk/settings/local.py.example`:  
  `cp techtalk/settings/local.py.example techtalk/settings/local.py`
...
- Open a new command line window and go to the project's directory.
- Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv techtalk` or `python -m venv techtalk-venv`
...
- Make sure the virtualenv is activated `workon techtalk` or `source techtalk-venv/bin/activate`
...
- `workon techtalk` or `source techtalk-venv/bin/activate` depending on if you are using virtualenvwrapper or just virtualenv.
